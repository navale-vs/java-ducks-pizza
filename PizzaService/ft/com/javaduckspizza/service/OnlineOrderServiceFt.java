...
public class OnlineOrderServiceFt {
	...
	public static void main(String[] args) {
		OnlineOrderServiceFt pizzaServiceFt = new OnlineOrderServiceFt();
		pizzaServiceFt.init();
	...
	private void getTypesById() throws Exception {
		...
		TypesVo tvGet = client.target(SERVICE_URL + "types/id/").path("/{id}").resolveTemplate("id", id).
	...
	private void getTypesBySequenceCode() throws Exception {
		...
		TypesVo tvGet = client.target(SERVICE_URL + "types/sequenceCode/").path("/{sequenceCode}").
...
//		tv.setCategory("CHES");
		...
		List<TypesVo> lst = client.target(SERVICE_URL + "types/category/").path("/{category}").
